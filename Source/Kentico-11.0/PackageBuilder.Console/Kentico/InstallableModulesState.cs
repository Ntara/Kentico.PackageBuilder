...
namespace CMS.Modules
{
    ...
    internal class InstallableModulesState
    {
        ...
        {
            mNuGetInstalledModuleVersions = CreateInstalledModulesDictionary();
        ...
        private Dictionary<string, string> CreateInstalledModulesDictionary()
        {
            var result = new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase);

            foreach (var nuGetInstalledModule in mNuGetInstalledModules)
            {
                try
                {
                    result.Add(nuGetInstalledModule.Name, nuGetInstalledModule.Version);
                }
                catch (ArgumentException)
                {
                    var moduleName = nuGetInstalledModule.Name;

                    throw new InvalidOperationException($"Collecting of installed modules failed. The module '{moduleName}' is present in more than one " +
                        $"version ({result[moduleName]} and {nuGetInstalledModule.Version}). This is typically a result of append only deployment after updating module to newer version. " +
                        $"All old module files must be removed when deploying an updated module.");
                }
            }
